...
- 可选导出文字集：单行去重字符集 `Config.txt`（固定包含英文大小写与 `0-9`）与去重文案清单 `Config.texts.txt`
- 支持基于文件哈希的增量导出
...
- `--json` 同时导出 JSON 文件
- `--text` 同时导出文字集：去重字符集 `Config.txt`（固定包含英文大小写与 `0-9`）与去重文案清单 `Config.texts.txt`
...
- 文案跨表去重，收集顺序为「表 → 行 → 字段」
- 字符集**固定包含英文大小写字母与 `0-9` 数字**（共 62 个）并置于最前。这些字符常由代码动态拼接（等级、分数、数量、道具 ID 等），不会出现在配表文案里，不固定纳入就可能在运行时出现缺字方块。固定字符在最前方也使 `Config.txt` 的前缀恒定，配表文案的增删只在尾部产生 diff
...
| 文件 | 内容 | 用途 |
|---|---|---|
| `Config.txt` | 全部文案中的**唯一字符**，并固定包含英文大小写与 `0-9`；单行输出，无分隔符与换行 | 直接作为 `font-subset -t` 的输入；每个字符只出现一次，可直观看出字符集规模 |
| `Config.texts.txt` | 去重后的**文案清单**，每条文案一行，末尾保留换行 | 核对哪些文案被纳入、排查漏字 |

`Config.txt` 收集的是**字符**而非文案，不保留文案边界；需要确认「某条文案是否被纳入」时请看 `Config.texts.txt`。`Config.texts.txt` 只记录配表文案，不含固定纳入的英文与数字。
